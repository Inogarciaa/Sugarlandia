    ...
    if (empty($email) || empty($password)) {
        ...
        if ($user && password_verify($password, $user['password'])) {
            if ($user['role'] === 'superadmin') {
                ...
                $_SESSION['superadmin_id']        = $user['user_id'];
                $_SESSION['superadmin_name']      = $user['name'];
                header("Location: index.php");
                exit;
            } else {
                $error = 'Access denied. Superadministrators only.';
            }
        } else {
...
<head>
  ...
  <title>Superadmin Login – Barquillio's</title>
...
<div class="wrapper">
  ...
  <h1><span>Superadmin</span> Login</h1>
  <p class="subtitle">Secure access for system configuration.</p>
  ...
  <div class="card">
    ...
    <form method="POST">
      <div class="field">
        ...
        <input type="email" name="email" placeholder="superadmin@example.com" value="<?= htmlspecialchars($_POST['email'] ?? '') ?>" required autofocus>
      </div>
      <div class="field">
